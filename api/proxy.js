...
export default async function handler(req, res) {
  ...
  try {
    // Extract the TMDB API path from the URL
    // Example: /api/trending/movie/week -> trending/movie/week
    const urlPath = req.url || '';
    const apiPrefix = '/api/';
    
    let tmdbPath = '';
    if (urlPath.startsWith(apiPrefix)) {
      // Remove /api/ prefix and extract path (before query string)
      const pathWithQuery = urlPath.slice(apiPrefix.length);
      const queryIndex = pathWithQuery.indexOf('?');
      tmdbPath = queryIndex >= 0 ? pathWithQuery.slice(0, queryIndex) : pathWithQuery;
    }
    
    // Remove 'proxy' from path if present (from rewrite)
    tmdbPath = tmdbPath.replace(/^proxy\/?/, '');
    ...
    // Get query parameters and add API key
